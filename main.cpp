...
std::vector<Bezier_curve> add_bundary(std::vector<Bezier_curve> curves){
    Bezier_curve B;

    B.push_back(Point_2(50,50));
    B.push_back(Point_2(50, 950));
    curves.push_back(B);

    B.clear();
    B.push_back(Point_2(50,950));
    B.push_back(Point_2(950, 950));
    curves.push_back(B);
    
    B.clear();
    B.push_back(Point_2(950,950));
    B.push_back(Point_2(950, 50));
    curves.push_back(B);
...
CDT triangulation(std::vector<Bezier_curve> curves){
    CDT             T;
    Vertex_handle   v1;
    Vertex_handle   v2;
...
refine_encroached(CDT T, std::vector < Bezier_curve > *curves, std::vector<std::pair<Point_2, double>> corners) {
    std::cout << "refine_encroached" << std::endl;
    int prev_n;
    int new_n;
constraint_refine:
...
void
protecting_balls(std::vector < Bezier_curve > curves, std::vector<std::pair<Point_2, double>> *corners){
    std::cout << "Generate protecting balls... ";
    auto C = get_corners(curves);
    int n_corner = C.size();
    ...
    for (int i=0; i<n_corner; i++){
        ...
        for(auto bezier : curves){
            for (auto p : bezier){
                if (ci.first != p && norm(ci.first-p) < ri){
                    ri = norm(ci.first-p);
                } 
            }
        }
        ((*corners)[i]).second = ri/4;
    }
    std::cout << "[OK]" << std::endl;
}
...
CDT split_constrained_triangles(CDT T){
...
int
draw(std::vector<Bezier_curve> curves, CDT T, int argc, char *argv[]){
    ...
    // polyPoints.clear();
    // for (typename CDT::Finite_vertices_iterator it=T.finite_vertices_begin();
    //      it!=T.finite_vertices_end(); ++it)
    //     polyPoints.push_back(QPoint(CGAL::to_double(it->point().x()), CGAL::to_double(it->point().y())));
